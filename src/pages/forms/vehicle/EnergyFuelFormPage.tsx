...
const EnergyFuelFormPage: React.FC = () => {
 ...
 return `${formatCurrency(finalTotal)} | ${parts.join(' | ')}`;;
 ...
 <MoneyInput id="pricePerLiter" name="pricePerLiter" label="Valor por Litro (R$)" placeholder="0,000" value={pricePerLiter} onChange={e => { setPricePerLiter(e.target.value); setLastEdited('price'); }} icon={<span className="text-sm font-semibold text-[var(--ios-text-secondary)]">R$</span>} />
 <FormInput id="liters" name="liters" label="Quantidade (L)" type="number" step="0.01" placeholder="0,00" value={liters} onChange={e => { setLiters(e.target.value); setLastEdited('quantity'); }} icon={<Droplets className="w-4 h-4 text-[var(--ios-text-tertiary)]" />} />
 <MoneyInput id="totalValue" name="totalValue" label="Valor Total (R$)" placeholder="0,00" value={totalValue} onChange={e => { setTotalValue(e.target.value); setLastEdited('total'); }} icon={<span className="text-sm font-semibold text-[var(--ios-text-secondary)]">R$</span>} />
 </FormSection>
 )}

 {showElectric && (
 <FormSection title={fuelType === 'Híbrido' ? "Parte 2: Energia Elétrica" : "Detalhes da Recarga"}>
 <MoneyInput id="pricePerKwh" name="pricePerKwh" label="Valor por kWh (R$)" placeholder="0,00" value={pricePerKwh} onChange={e => setPricePerKwh(e.target.value)} icon={<span className="text-sm font-semibold text-[var(--ios-text-secondary)]">R$</span>} />
 <FormInput id="kwh" name="kwh" label="Quantidade (kWh)" type="number" step="0.01" placeholder="0,0" value={kwh} onChange={e => setKwh(e.target.value)} icon={<Zap className="w-4 h-4 text-[var(--ios-text-tertiary)]" />} />
 <MoneyInput id="totalElectricValue" name="totalElectricValue" label="Valor Total da Recarga (R$)" placeholder="0,00" value={totalElectricValue} onChange={e => setTotalElectricValue(e.target.value)} icon={<span className="text-sm font-semibold text-[var(--ios-text-secondary)]">R$</span>} />
 </FormSection>
 )}

  <FormSection title="Informações Gerais">
 <VehicleSelector value={vehicleId} onChange={setVehicleId} />
 <FormInput id="date" name="date" label="Data do Abastecimento" type="date" value={date} onChange={e => setDate(e.target.value)} required icon={<Calendar className="w-4 h-4 text-[var(--ios-text-tertiary)]" />} />
 <FormInput id="odometer" name="odometer" label="Odômetro Atual (km)" type="number" placeholder="50123" value={odometer} onChange={e => setOdometer(e.target.value)} required icon={<Gauge className="w-4 h-4 text-[var(--ios-text-tertiary)]" />} />
