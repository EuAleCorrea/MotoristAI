...
const MaintenanceFormPage: React.FC = () => {
 ...
 <MoneyInput id="partsCost" name="partsCost" label="Custo de Peças (R$)" placeholder="0,00" value={partsCost} onChange={e => setPartsCost(e.target.value)} icon={<span className="text-sm font-semibold text-[var(--ios-text-secondary)]">R$</span>} />
 <MoneyInput id="laborCost" name="laborCost" label="Custo de Mão de Obra (R$)" placeholder="0,00" value={laborCost} onChange={e => setLaborCost(e.target.value)} icon={<span className="text-sm font-semibold text-[var(--ios-text-secondary)]">R$</span>} />
 <MoneyInput id="totalValue" name="totalValue" label="Valor Total (R$)" value={totalValue} readOnly disabled icon={<span className="text-sm font-semibold text-[var(--ios-text-secondary)]">R$</span>} />
 <div>
 <FormInput id="odometer" name="odometer" label="Odômetro Atual (km)" type="number" placeholder="51200" value={odometer} onChange={e => setOdometer(e.target.value)} required icon={<Gauge className="w-4 h-4 text-[var(--ios-text-tertiary)]" />} />
 {odometerWarning && (
 <div className="mt-2 flex items-center text-sm text-[var(--sys-yellow)] bg-[rgba(255,204,0,0.08)] p-2 rounded-md">
 ...
 <label className="block text-sm font-medium text-[var(--ios-text)] mb-1.5">Nota Fiscal / Imagem</label>
 <label htmlFor="invoice-upload" className="w-full flex items-center justify-center px-4 py-3 border-2 border-dashed border-[var(--ios-separator)] rounded-lg cursor-pointer hover:border-[var(--ios-accent)] hover:bg-[var(--ios-fill)] dark:hover:bg-primary-900/10 transition">
 <Paperclip className="w-5 h-5 text-[var(--ios-text-secondary)]" />
 <span className="text-sm font-medium text-[var(--ios-text-secondary)] ml-2">{invoiceFile ? invoiceFile.name : 'Clique para anexar um arquivo'}</span>
 </label>
 <input id="invoice-upload" type="file" className="hidden" onChange={handleFileChange} accept="image/*,.pdf" />
 <p className="text-xs text-[var(--ios-text-secondary)] mt-1">A funcionalidade de upload requer integração com um serviço de armazenamento.</p>
 </div>
