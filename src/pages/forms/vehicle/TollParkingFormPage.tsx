...
const TollParkingFormPage: React.FC = () => {
 ...
 {expenseType === 'Pedágio' && (
 <FormInput id="highway" name="highway" label="Rodovia (Opcional)" type="text" placeholder="Ex: SP-330" value={highway} onChange={e => setHighway(e.target.value)} icon={<Route className="w-4 h-4 text-[var(--ios-text-tertiary)]" />} />
 )}
 {expenseType === 'Estacionamento' && (
 <FormInput id="duration" name="duration" label="Permanência (minutos)" type="number" placeholder="120" value={duration} onChange={e => setDuration(e.target.value)} icon={<Clock className="w-4 h-4 text-[var(--ios-text-tertiary)]" />} />
 )}
