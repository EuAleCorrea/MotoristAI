...
const LeisureFormPage: React.FC = () => {
 ...
 {expenseType === 'Viagem' && (
 <>
 <FormInput id="destination" label="Destino" type="text" placeholder="Ex: Praia Grande, SP" value={destination} onChange={e => setDestination(e.target.value)} icon={<MapPin className="w-4 h-4 text-[var(--ios-text-tertiary)]" />} />
 <FormInput id="durationInDays" label="Duração (dias)" type="number" placeholder="7" value={durationInDays} onChange={e => setDurationInDays(e.target.value)} icon={<Clock className="w-4 h-4 text-[var(--ios-text-tertiary)]" />} />
 </>
 )}
 </FormSection>
 </motion.div>
