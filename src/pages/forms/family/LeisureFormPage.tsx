...
import { Drama, Calendar, DollarSign, MapPin, Users, Plane, Film, Utensils, Mountain, Tv, Dumbbell, MoreHorizontal, Repeat, Clock } from 'lucide-react';
...
const LeisureFormPage: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    alert('Despesa de lazer salva com sucesso!');
    navigate(-1);
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 pb-24">
      <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 flex justify-between items-start">
        <div>
          <h3 className="text-lg font-semibold text-gray-900 mb-2">Despesa de Lazer</h3>
          <p className="text-sm text-gray-500">Registre gastos com entretenimento e diversão.</p>
        </div>
        <Drama className="w-8 h-8 text-teal-500" />
      </div>
      ...
      <FormSection title="Valores e Participantes">
        <FormInput id="totalValue" label="Valor Total (R$)" type="number" step="0.01" placeholder="150.00" value={totalValue} onChange={e => setTotalValue(e.target.value)} required icon={<DollarSign className="w-4 h-4 text-gray-400" />} />
