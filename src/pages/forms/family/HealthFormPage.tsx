...
const HealthFormPage: React.FC = () => {
 ...
 >
 <MoneyInput id="reimbursementValue" label="Valor do Reembolso (R$)" placeholder="0,00" value={reimbursementValue} onChange={e => setReimbursementValue(e.target.value)} icon={<span className="text-sm font-semibold text-[var(--ios-text-secondary)]">R$</span>} />
 <FormInput id="reimbursementDate" label="Data do Reembolso" type="date" value={reimbursementDate} onChange={e => setReimbursementDate(e.target.value)} icon={<Calendar className="w-4 h-4 text-[var(--ios-text-tertiary)]" />} />
 </motion.div>
 )}
 </AnimatePresence>
 <div className="md:col-span-2 bg-[var(--ios-fill)] border-l-4 border-[var(--sys-blue)] p-4 rounded-r-lg">
