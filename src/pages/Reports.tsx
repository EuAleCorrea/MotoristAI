...
const Reports = () => {
 ...
 case 'category':
 return (
 <div className="bg-[var(--ios-card)] rounded-xl shadow-sm p-6">
 <h2 className="text-lg font-semibold text-[var(--ios-text)] mb-4">Despesas por Categoria</h2>
 {Object.keys(expensesByCategory).length > 0 ? (
 <ReactECharts option={categoryChartOption} style={{ height: '350px' }} />
 ) : (
 <div className="flex items-center justify-center h-64 text-[var(--ios-text-secondary)]">Nenhuma despesa registrada</div>
 )}
 </div>
 );
 case 'km_energy':
 ...
 <div className="bg-[var(--ios-card)] rounded-xl shadow-sm p-6 text-center">
 <p className="text-sm text-[var(--ios-text-secondary)]">Total Gasto (Combustível/Energia)</p>
 <p className="text-3xl font-bold text-[var(--ios-accent)] mt-2">{formatCurrency(kmEnergyData.totalFuelCost)}</p>
 </div>
 <div className="bg-[var(--ios-card)] rounded-xl shadow-sm p-6 text-center">
 <p className="text-sm text-[var(--ios-text-secondary)]">Km Rodados (Período)</p>
 <p className="text-3xl font-bold text-[var(--ios-text)] mt-2">{formatInteger(kmEnergyData.totalKm)} km</p>
 </div>
 <div className="bg-[var(--ios-card)] rounded-xl shadow-sm p-6 text-center">
 <p className="text-sm text-[var(--ios-text-secondary)]">Custo por Km</p>
 <p className="text-3xl font-bold text-success-600 mt-2">{formatNumber(kmEnergyData.costPerKm)}/km</p>
 </div>
 </div>
 <div className="bg-[rgba(255,204,0,0.08)] border-l-4 border-[var(--sys-yellow)] p-4 rounded-r-lg">
 <p className="text-sm text-[var(--sys-yellow)] dark:text-yellow-300">Para dados mais precisos, certifique-se de registrar a quilometragem em cada lançamento de receita.</p>
 </div>
 </div>
 );
 case 'platforms':
 return (
 <div className="bg-[var(--ios-card)] rounded-xl shadow-sm p-6">
 <h2 className="text-lg font-semibold text-[var(--ios-text)] mb-4">Receita por Plataforma</h2>
 {Object.keys(revenueByPlatform).length > 0 ? (
 <ReactECharts option={platformChartOption} style={{ height: '350px' }} />
 ) : (
 <div className="flex items-center justify-center h-64 text-[var(--ios-text-secondary)]">Nenhuma receita registrada</div>
 )}
 </div>
 );
 case 'monthly':
 ...
 <div className="space-y-6">
 <div className="bg-[var(--ios-card)] rounded-xl shadow-sm p-6">
 <h2 className="text-lg font-semibold text-[var(--ios-text)] mb-4">Receita, Despesas e Lucro Mensal</h2>
 <ReactECharts option={monthlyChartOption} style={{ height: '400px' }} />
 </div>
 <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
 <div className="p-4 bg-[rgba(52,199,89,0.08)] rounded-lg">
 ...
 <div className="p-4 bg-[rgba(255,59,48,0.08)] rounded-lg">
 ...
 <div className="p-4 bg-[var(--ios-fill)] rounded-lg">
 <p className="text-sm text-[var(--ios-accent)] font-medium">Lucro Total</p>
 <p className="mt-1 text-2xl font-bold text-[var(--ios-accent)] ">{formatCurrency(monthsData.reduce((sum, d) => sum + d.profit, 0))}</p>
 </div>
 <div className="p-4 bg-[var(--ios-bg)] rounded-lg">
 <p className="text-sm text-[var(--ios-text-secondary)] font-medium">Total de Viagens</p>
 <p className="mt-2 text-2xl font-bold text-[var(--ios-text)] ">{monthsData.reduce((sum, d) => sum + d.trips, 0)}</p>
 </div>
 </div>
 </div>
 );
 ...
 return null;
  ...
  return (
    <div className="space-y-6">
      {/* Banner - Comprovante de Renda */}
      <button
        onClick={() => navigate('/relatorios/comprovante-renda')}
        className="w-full flex items-center gap-4 p-4 bg-gradient-to-r from-[var(--ios-accent)] to-blue-600 rounded-xl shadow-sm hover:opacity-95 transition-all group"
      >
        <div className="p-3 bg-white/20 rounded-xl group-hover:scale-110 transition-transform">
          <Download className="h-6 w-6 text-white" />
        </div>
        <div className="text-left flex-1">
          <h3 className="font-semibold text-white">Comprovante de Renda</h3>
          <p className="text-sm text-white/80">Gere um PDF profissional para comprovação de renda</p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-[var(--ios-text)]">{viewConfig[activeView].title}</h1>
