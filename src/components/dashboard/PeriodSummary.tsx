    ...
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow-sm p-4 flex justify-between items-center">
        <div>
          ...
          <p className="text-2xl font-bold text-primary-600">{performance.toFixed(0)}%</p>
        </div>
        <div className="text-right">
          <p className="text-sm font-medium text-gray-500">Meta do Período</p>
          <p className="text-2xl font-bold text-danger-600">R$ {periodGoal.toFixed(2)}</p>
        </div>
      </div>
      
      <div className="bg-white rounded-lg shadow-sm p-4">
        <div className="flex justify-between text-sm font-medium mb-1">
          <span className="text-gray-700">Faturamento: R$ {revenue.toFixed(2)}</span>
        </div>
        <ProgressBar value={revenue} max={periodGoal} color="bg-green-500" />
        <div className="flex justify-end text-sm font-medium mt-1">
          <span className="text-gray-500">Pendente: R$ {Math.max(0, periodGoal - revenue).toFixed(2)}</span>
        </div>
      </div>

      <div className="bg-white rounded-lg shadow-sm p-6 text-center">
        <p className="text-md font-medium text-gray-600">Faturamento Total</p>
        <p className="text-4xl font-bold text-gray-900 mt-1">R$ {revenue.toFixed(2)}</p>
      </div>
      <div className="grid grid-cols-2 gap-4">
        <div className="text-center p-4 bg-danger-50 rounded-lg">
          <p className="text-sm font-medium text-danger-600">Despesas</p>
          <p className="text-xl sm:text-2xl font-bold text-danger-700">R$ {expenseTotal.toFixed(2)}</p>
        </div>
        <div className="text-center p-4 bg-primary-50 rounded-lg">
          <p className="text-sm font-medium text-primary-600">Saldo</p>
          <p className="text-xl sm:text-2xl font-bold text-primary-700">R$ {balance.toFixed(2)}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm p-4">
        <h3 className="text-md font-semibold text-gray-800 mb-3">Faturamento por Aplicativo</h3>
        <div className="space-y-3">
          ...
          )) : <p className="text-center text-gray-500 text-sm py-4">Nenhuma corrida no período.</p>}
        </div>
      </div>

       <div className="bg-white rounded-lg shadow-sm">
        <div className="flex justify-between items-center p-4 border-b">
            <h3 className="text-md font-semibold text-gray-800">Despesas</h3>
            <div className="flex items-center">
                <label htmlFor="expense-toggle" className="text-sm text-gray-600 mr-2">Todas</label>
        ...
        <div className="p-4">
            {expensesToDisplay.length > 0 ? (
                <ul className="space-y-3">
                    {expensesToDisplay.map(expense => (
                        <li key={expense.id} className="flex items-center justify-between">
                            ...
                            <p className="font-semibold text-danger-600">- R$ {expense.amount.toFixed(2)}</p>
                        </li>
                    ))}
                ...
                <p className="text-center text-gray-500 text-sm py-4">Nenhuma despesa no período.</p>
            )}
        </div>
      </div>
    </div>
