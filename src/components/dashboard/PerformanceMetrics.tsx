...
const MetricCard: React.FC<MetricCardProps> = ({ icon, label, value, subLabel, trend }) => (
    <div className="bg-slate-100 dark:bg-slate-800/50 rounded-xl p-4 flex items-center gap-3 border border-slate-200 dark:border-slate-700/50">
        ...
        {trend && (
            <div className={`text-xs ${trend === 'up' ? 'text-green-500 dark:text-green-400' : trend === 'down' ? 'text-red-500 dark:text-red-400' : 'text-slate-400'}`}>
...
function PerformanceMetrics() {
    ...
    return (
        <div className="space-y-3">
            <h3 className="text-sm font-medium text-slate-600 dark:text-slate-400 px-1">Métricas de Performance (Semana)</h3>
